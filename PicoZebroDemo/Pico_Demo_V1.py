...
# Pico Demo Version 1 with 10 Pico Zebro's
# Full Code for controlling and Detecting 10 Pico Zebro 
    ...
    def run(self):                                                          # run Uart Thread
        while True:                                                         # Run UArt thread untill main ends.
            ...
            if (self.q_Control_Serial_Write.empty() == False) or (Sended_Data == 1): # Check if there is data to send.
                ...
                if Serial[1][0] == "Main":                                  # If main has to send data 
                    if Serial[1][1] == 'Global':                            # If it has to be send to each device
                        if Serial[1][2] == 'Leds_off':                      # if the global command is leds off.
                            for x in range(10):                             # Go over each Zebro and set it the leds to 0
                                print(x)
                        ...
                        if Serial[1][2] == 'Stop':                          # if the global command is Stop
                            for x in range(10):                             # Make every Zebro stop
                        ...
                        if Serial[1][2] == 'Led3_on':
                            ...
                            setLed(connectionID=0, ledNr=3, value=0)
                            setLed(connectionID=0, ledNr=2, value=0)
                    elif Serial[1][1] == 'Pico_N9':                         # This has to made for every Pico Zebro for controlling it seperatly with the Main
                        if Serial[1][2] == 'Led1_on':                       # The three used commands are Led1_on
                            setLed(connectionID=0, ledNr=3, value=7)        # Led3_on and Leds_off 
                        if Serial[1][2] == 'Led3_on':
                            setLed(connectionID=0, ledNr=2, value=7)
                            ...
                            setLed(connectionID=0, ledNr=3, value=0)
                            setLed(connectionID=0, ledNr=2, value=0)
                    elif Serial[1][1] == 'Pico_N10':                         # This has to made for every Pico Zebro for controlling it seperatly with the Main
                        if Serial[1][2] == 'Led1_on':                       # The three used commands are Led1_on
                            setLed(connectionID=0, ledNr=3, value=7)        # Led3_on and Leds_off 
                        if Serial[1][2] == 'Led3_on':
                            setLed(connectionID=0, ledNr=2, value=7)
                ...
                elif Serial[1][0] == "Pico_N2":                             # if Pico_N2 has to send data for now only four commands
                    if Serial[1][1] == "Pico_N2":
                        connection_Pico_1 = 0
                        if Serial[1][2] == "Stop":
                            setMovement(connection_Pico_1, 0)
                        elif Serial[1][2] == "Forward":
                        ...
                        print("This is the wrong Pico")                     # Extra check for if it is send to the wrong Pico
                    Sended_Data = 0
                    print("Pico_Zebro_2_Wrote")
                elif Serial[1][0] == "Pico_N3":                             # if Pico_N3 has to send data for now only four commands
                    if Serial[1][1] == "Pico_N3":
                        connection_Pico_1 = 0
                        if Serial[1][2] == "Stop":
                            setMovement(connection_Pico_1, 0)
                        elif Serial[1][2] == "Forward":
                        ...
                        print("This is the wrong Pico")                     # Extra check for if it is send to the wrong Pico
                    Sended_Data = 0
                    print("Pico_Zebro_3_Wrote")
                elif Serial[1][0] == "Pico_N4":                             # if Pico_N4 has to send data for now only four commands
                    if Serial[1][1] == "Pico_N4":
                        connection_Pico_1 = 0
                        if Serial[1][2] == "Stop":
                            setMovement(connection_Pico_1, 0)
                        elif Serial[1][2] == "Forward":
                        ...
                        print("This is the wrong Pico")                     # Extra check for if it is send to the wrong Pico
                    Sended_Data = 0
                    print("Pico_Zebro_4_Wrote")
                elif Serial[1][0] == "Pico_N5":                             # if Pico_N5 has to send data for now only four commands
                    if Serial[1][1] == "Pico_N5":
                        connection_Pico_1 = 0
                        if Serial[1][2] == "Stop":
                            setMovement(connection_Pico_1, 0)
                        elif Serial[1][2] == "Forward":
                        ...
                        print("This is the wrong Pico")                     # Extra check for if it is send to the wrong Pico
                    Sended_Data = 0
                    print("Pico_Zebro_5_Wrote")
                elif Serial[1][0] == "Pico_N6":                             # if Pico_N6 has to send data for now only four commands
                    if Serial[1][1] == "Pico_N6":
                        connection_Pico_1 = 0
                        if Serial[1][2] == "Stop":
                            setMovement(connection_Pico_1, 0)
                        elif Serial[1][2] == "Forward":
                        ...
                        print("This is the wrong Pico")                     # Extra check for if it is send to the wrong Pico
                    Sended_Data = 0
                    print("Pico_Zebro_6_Wrote")
                elif Serial[1][0] == "Pico_N7":                             # if Pico_N7 has to send data for now only four commands
                    if Serial[1][1] == "Pico_N7":
                        connection_Pico_1 = 0
                        if Serial[1][2] == "Stop":
                            setMovement(connection_Pico_1, 0)
                        elif Serial[1][2] == "Forward":
                        ...
                        print("This is the wrong Pico")                     # Extra check for if it is send to the wrong Pico
                    Sended_Data = 0
                    print("Pico_Zebro_7_Wrote")
                elif Serial[1][0] == "Pico_N8":                             # if Pico_N8 has to send data for now only four commands
                    if Serial[1][1] == "Pico_N8":
                        connection_Pico_1 = 0
                        if Serial[1][2] == "Stop":
                            setMovement(connection_Pico_1, 0)
                        elif Serial[1][2] == "Forward":
                        ...
                        print("This is the wrong Pico")                     # Extra check for if it is send to the wrong Pico
                    Sended_Data = 0
                    print("Pico_Zebro_8_Wrote")
                elif Serial[1][0] == "Pico_N9":                             # if Pico_N9 has to send data for now only four commands
                    if Serial[1][1] == "Pico_N9":
                        connection_Pico_1 = 0
                        if Serial[1][2] == "Stop":
                            setMovement(connection_Pico_1, 0)
                        elif Serial[1][2] == "Forward":
                        ...
                        print("This is the wrong Pico")                     # Extra check for if it is send to the wrong Pico
                    Sended_Data = 0
                    print("Pico_Zebro_9_Wrote")
                elif Serial[1][0] == "Pico_N10":                             # if Pico_N10 has to send data for now only four commands
                    if Serial[1][1] == "Pico_N10":
                        connection_Pico_1 = 0
                        if Serial[1][2] == "Stop":
                            setMovement(connection_Pico_1, 0)
                        elif Serial[1][2] == "Forward":
...
class Control_Zebro_Thread(threading.Thread):                               # The Control Pico Zebro Thread. For each possible Zebro A thread is created.
    ...
    def run(self):
        ...
        if self.Zebro == 'Pico_N8':                                         # Elif for every Zebro + 1
            Connected_To = 7
        if self.Zebro == 'Pico_N9':                                         # Elif for every Zebro + 1
            Connected_To = 8
        if self.Zebro == 'Pico_N10':                                        # Elif for every Zebro + 1
            Connected_To = 9
        ...
        DONT_Send = 0                                                       # if the way is blocked
        ...
        while True:                                                         # Run thread until Main has ended
            ...
            if Connected == 1:
                ...
                try: 
                    ...
                    Middle_point_y = PicoZebro[1]
                    Blocked_Direction = PicoZebro[2]
                except queue.Empty:                                         # if the queue is empty use old data (Change this happens is extremely small when connected)
...
def main(q_Control_Serial_Write,q_Data_is_Send,q_Control_Uart_Main,         # The main in here for every possible Zebro the Direction, Angle and middlepoint queue needs to be given
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):  # BGR is the standard way for OpenCV
        ...
        if Picture == 1:
            ...
            time.sleep(1)                                                   # Give the command some time to finish with turning all leds off.
        ...
        if Picture == 6:
            ...
            Difference_led_1 = Image_Difference(New_image_Led_1)            # Make only the led 1 visible
        ...
        if Picture == 7:
            ...
            for Zebros in range(10):                                         # total of maximum of x Zebro's
                Blocking_Zebro = []   #Here will be the blocking in
                if Zebros == 0:                                             # The comming step has to be done for every possible Zebro 
                    
                    # In here should be every middle point possible for every Zebro. Block_2 or Block_1 or maybe 3 needs to be adjusted accordingly
                    Blocking_Zebro = Block.Block_3(Zebro_1_Middle_x, Zebro_2_Middle_x, Zebro_3_Middle_x, Zebro_4_Middle_x, Zebro_5_Middle_x, Zebro_6_Middle_x, Zebro_7_Middle_x, Zebro_8_Middle_x, Zebro_9_Middle_x, Zebro_10_Middle_x,
                    ...
                    PicoZebro_1 = [Zebro_1_Middle_x , Zebro_1_Middle_y, Blocking_Zebro] # Set PicoZebro_1 with new values
                ...
                if Zebros == 1:
                    ...
                    Blocking_Zebro = Block.Block_3(Zebro_2_Middle_x, Zebro_1_Middle_x, Zebro_3_Middle_x, Zebro_4_Middle_x, Zebro_5_Middle_x, Zebro_6_Middle_x, Zebro_7_Middle_x, Zebro_8_Middle_x, Zebro_9_Middle_x, Zebro_10_Middle_x,
                ...
                if Zebros == 2:
                    # In here should be every middle point possible for every Zebro. Block_2 or Block_1 or maybe 3 needs to be adjusted accordingly
                    Blocking_Zebro = Block.Block_3(Zebro_3_Middle_x, Zebro_2_Middle_x, Zebro_1_Middle_x, Zebro_4_Middle_x, Zebro_5_Middle_x, Zebro_6_Middle_x, Zebro_7_Middle_x, Zebro_8_Middle_x, Zebro_9_Middle_x, Zebro_10_Middle_x,
                ...
                if Zebros == 3:
                    # In here should be every middle point possible for every Zebro. Block_2 or Block_1 or maybe 3 needs to be adjusted accordingly
                    Blocking_Zebro = Block.Block_3(Zebro_4_Middle_x, Zebro_2_Middle_x, Zebro_3_Middle_x, Zebro_1_Middle_x, Zebro_5_Middle_x, Zebro_6_Middle_x, Zebro_7_Middle_x, Zebro_8_Middle_x, Zebro_9_Middle_x, Zebro_10_Middle_x,
                ...
                if Zebros == 4:
                    # In here should be every middle point possible for every Zebro. Block_2 or Block_1 or maybe 3 needs to be adjusted accordingly
                    Blocking_Zebro = Block.Block_3(Zebro_5_Middle_x, Zebro_2_Middle_x, Zebro_3_Middle_x, Zebro_4_Middle_x, Zebro_1_Middle_x, Zebro_6_Middle_x, Zebro_7_Middle_x, Zebro_8_Middle_x, Zebro_9_Middle_x, Zebro_10_Middle_x,
                ...
                if Zebros == 5:
                    # In here should be every middle point possible for every Zebro. Block_2 or Block_1 or maybe 3 needs to be adjusted accordingly
                    Blocking_Zebro = Block.Block_3(Zebro_6_Middle_x, Zebro_2_Middle_x, Zebro_3_Middle_x, Zebro_4_Middle_x, Zebro_5_Middle_x, Zebro_1_Middle_x, Zebro_7_Middle_x, Zebro_8_Middle_x, Zebro_9_Middle_x, Zebro_10_Middle_x,
                ...
                if Zebros == 6:
                    # In here should be every middle point possible for every Zebro. Block_2 or Block_1 or maybe 3 needs to be adjusted accordingly
                    Blocking_Zebro = Block.Block_3(Zebro_7_Middle_x, Zebro_2_Middle_x, Zebro_3_Middle_x, Zebro_4_Middle_x, Zebro_5_Middle_x, Zebro_6_Middle_x, Zebro_1_Middle_x, Zebro_8_Middle_x, Zebro_9_Middle_x, Zebro_10_Middle_x,
                ...
                if Zebros == 7:
                    # In here should be every middle point possible for every Zebro. Block_2 or Block_1 or maybe 3 needs to be adjusted accordingly
                    Blocking_Zebro = Block.Block_3(Zebro_8_Middle_x, Zebro_2_Middle_x, Zebro_3_Middle_x, Zebro_4_Middle_x, Zebro_5_Middle_x, Zebro_6_Middle_x, Zebro_7_Middle_x, Zebro_1_Middle_x, Zebro_9_Middle_x, Zebro_10_Middle_x,
                ...
                if Zebros == 8:
                    # In here should be every middle point possible for every Zebro. Block_2 or Block_1 or maybe 3 needs to be adjusted accordingly
                    Blocking_Zebro = Block.Block_3(Zebro_9_Middle_x, Zebro_2_Middle_x, Zebro_3_Middle_x, Zebro_4_Middle_x, Zebro_5_Middle_x, Zebro_6_Middle_x, Zebro_7_Middle_x, Zebro_8_Middle_x, Zebro_1_Middle_x, Zebro_10_Middle_x,
                ...
                if Zebros == 9:
                    # In here should be every middle point possible for every Zebro. Block_2 or Block_1 or maybe 3 needs to be adjusted accordingly
                    Blocking_Zebro = Block.Block_3(Zebro_10_Middle_x, Zebro_2_Middle_x, Zebro_3_Middle_x, Zebro_4_Middle_x, Zebro_5_Middle_x, Zebro_6_Middle_x, Zebro_7_Middle_x, Zebro_8_Middle_x, Zebro_9_Middle_x, Zebro_1_Middle_x,
