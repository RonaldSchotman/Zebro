...
for frame in camera.capture_continuous(rawCapture,format="bgr",use_video_port=True):
	image = frame.array
     ...
     image_gray = cv2.cvtColor(image, cv2.BGR2GRAY) # Convert Image captured from Image Input to GrayScale
    	edges = cv2.Canny(image_gray,100,200)      # Apply Canny edge detection on the gray image
	contours,hierarchy = cv2.findContours(edges,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE) # Find contours with hierarchy
	...
	if (mark >= 3):    # Ensure we have (atleast 3; namely A,B,C) 'Alignment Markers' discovered
		...
		if(top < len(contours) and right < len(contours) and bottom < len(contours) and cv2.contourArea(contours[top]) > 10 and cv2.contourArea(contours[right]) > 10 and cv2.contourArea(contours[bottom]) > 10):
			...
			cv2.drawContours(image,contours,bottom,(0,0,255),2)
               ...
               if (orientation == 0):
                   cv2.putText(image,'North',Point(20,30),0,0.3,(0,255,0)
               elif (orientation == 1):
                   cv2.putText(image,'East',Point(20,30),0,0.3,(0,255,0)
               elif (orientation == 2):
                   cv2.putText(image,'South',Point(20,30),0,0.3,(0,255,0)
               elif (orientation == 3):
                   cv2.putText(image,'West',Point(20,30),0,0.3,(0,255,0)                   
