...
from Functions.Blocking2 import Blocking2                                     # py program for comparing every zebro middlepoint with eachother

Block = Blocking2()                                                          # For using the functions in Blocking.py with Block.Function_Name
                ...
                if Serial[1][0] == "Main":                                  # If main has to send data 
                    if Serial[1][1] == 'Global':                            # If it has to be send to each device
                        if Serial[1][2] == 'Leds_off':                      # if the global command is leds off.
                            for x in range(6):                              # Go over each Zebro and set it the leds to 0
                                #print(x)
                            ...
                            for x in range(6):                              # A loop over every possible Zebro
...
class Control_Zebro_Thread(threading.Thread):                               # The Control Pico Zebro Thread. For each possible Zebro A thread is created.
    ...
    def run(self):
        ...
        if self.Zebro == 'Pico_N1':                                         # Elif for every Zebro + 1
            ...
            Connected_To = 1                                                # Connected_To for Pico_N2 is then: 1
        elif self.Zebro == 'Pico_N4':                                         # Elif for every Zebro + 1
            Connected_To = 1                                                # Connected_To for Pico_N2 is then: 1
        elif self.Zebro == 'Pico_N5':                                         # Elif for every Zebro + 1
            Connected_To = 1                                                # Connected_To for Pico_N2 is then: 1
        elif self.Zebro == 'Pico_N6':                                         # Elif for every Zebro + 1
            Connected_To = 1                                                # Connected_To for Pico_N2 is then: 1
...
def main(q_Control_Serial_Write,q_Data_is_Send,q_Control_Uart_Main,         # The main in here for every possible Zebro the Direction, Angle and middlepoint queue needs to be given
         q_PicoZebro_1, q_PicoZebro_2,q_PicoZebro_3,q_PicoZebro_4,q_PicoZebro_5,q_PicoZebro_6,
         q_Pico_Direction_1, q_Pico_Direction_2, q_Pico_Direction_3,q_Pico_Direction_4,q_Pico_Direction_5,q_Pico_Direction_6,
         q_Pico_Angle_1, q_Pico_Angle_2, q_Pico_Angle_3, q_Pico_Angle_4, q_Pico_Angle_5, q_Pico_Angle_6):
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):  # BGR is the standard way for OpenCV
        ...
        if Picture == 2:                                                    # Step 1 for Pico Control is taking the original picture and taking control of thread UART Control
            ...
            Zebro_3_Middle_x = 0                                            # Reset all values for calculations. This needs to be done for every Zebro
            Zebro_3_Middle_y = 0
            PicoZebro_3 = []

            Zebro_4_Middle_x = 0                                            # Reset all values for calculations. This needs to be done for every Zebro
            Zebro_4_Middle_y = 0
            PicoZebro_4 = []

            Zebro_5_Middle_x = 0                                            # Reset all values for calculations. This needs to be done for every Zebro
            Zebro_5_Middle_y = 0
            PicoZebro_5 = []

            Zebro_6_Middle_x = 0                                            # Reset all values for calculations. This needs to be done for every Zebro
            Zebro_6_Middle_y = 0
            PicoZebro_6 = []
        ...
        if Picture == 8:
            ...
            for Zebros in range(6):                                         # total of maximum of x Zebro's
                ...
                if Zebros == 0:                                             # The comming step has to be done for every possible Zebro 
                    ...
                    Blocking_Zebro = Block.Block_9(Zebro_1_Middle_x, Zebro_2_Middle_x, Zebro_3_Middle_x, Zebro_4_Middle_x, Zebro_5_Middle_x, Zebro_6_Middle_x,
                    ...
                    if q_Pico_Direction_1.empty() == True:                  # if the queue is empty fill it
                        ...
                        q_Pico_Direction_1.put(Direction_Zebro_1)
                if Zebros == 1:                                             # The comming step has to be done for every possible Zebro 
                    # The reason Block function is in another python script is to make this one easier to read because the BLocked functions got a lot of if statements. 
                    Blocking_Zebro = Block.Block_9(Zebro_2_Middle_x, Zebro_1_Middle_x, Zebro_3_Middle_x, Zebro_4_Middle_x, Zebro_5_Middle_x, Zebro_6_Middle_x,
                                                   Zebro_2_Middle_y, Zebro_1_Middle_y, Zebro_3_Middle_y, Zebro_4_Middle_y, Zebro_5_Middle_y, Zebro_6_Middle_y) 
                    ...
                    if q_Pico_Direction_2.empty() == True:                  # if the queue is empty fill it
                        ...
                        q_Pico_Direction_2.put(Direction_Zebro_2)
                if Zebros == 2:                                             # The comming step has to be done for every possible Zebro 
                    # The reason Block function is in another python script is to make this one easier to read because the BLocked functions got a lot of if statements. 
                    Blocking_Zebro = Block.Block_9(Zebro_3_Middle_x, Zebro_2_Middle_x, Zebro_1_Middle_x, Zebro_4_Middle_x, Zebro_5_Middle_x, Zebro_6_Middle_x,
                                                   Zebro_3_Middle_y, Zebro_2_Middle_y, Zebro_1_Middle_y, Zebro_4_Middle_y, Zebro_5_Middle_y, Zebro_6_Middle_y) 
                    ...
                    if q_Pico_Direction_3.empty() == True:                  # if the queue is empty fill it
                        ...
                        q_Pico_Direction_3.put(Direction_Zebro_3)
                if Zebros == 3:                                             # The comming step has to be done for every possible Zebro 
                    # The reason Block function is in another python script is to make this one easier to read because the BLocked functions got a lot of if statements. 
                    Blocking_Zebro = Block.Block_9(Zebro_4_Middle_x, Zebro_2_Middle_x, Zebro_3_Middle_x, Zebro_1_Middle_x, Zebro_5_Middle_x, Zebro_6_Middle_x,
                                                   Zebro_4_Middle_y, Zebro_2_Middle_y, Zebro_3_Middle_y, Zebro_1_Middle_y, Zebro_5_Middle_y, Zebro_6_Middle_y) 
                ...
                if Zebros == 4:                                             # The comming step has to be done for every possible Zebro 
                    # The reason Block function is in another python script is to make this one easier to read because the BLocked functions got a lot of if statements. 
                    Blocking_Zebro = Block.Block_9(Zebro_5_Middle_x, Zebro_2_Middle_x, Zebro_3_Middle_x, Zebro_4_Middle_x, Zebro_1_Middle_x, Zebro_6_Middle_x,
                                                   Zebro_5_Middle_y, Zebro_2_Middle_y, Zebro_3_Middle_y, Zebro_4_Middle_y, Zebro_1_Middle_y, Zebro_6_Middle_y) 
                    ...
                    # The reason Block function is in another python script is to make this one easier to read because the BLocked functions got a lot of if statements. 
                    Blocking_Zebro = Block.Block_9(Zebro_6_Middle_x, Zebro_2_Middle_x, Zebro_3_Middle_x, Zebro_4_Middle_x, Zebro_5_Middle_x, Zebro_1_Middle_x,
                                                   Zebro_6_Middle_y, Zebro_2_Middle_y, Zebro_3_Middle_y, Zebro_4_Middle_y, Zebro_5_Middle_y, Zebro_1_Middle_y) 
        ...
        if Picture == 9:
            ...
            for (i, c) in enumerate(cnts):
                ...
                Old_Zebro_4_Middle_x_pos = Old_Zebro_4_Middle_x + 200
                Old_Zebro_4_Middle_x_neg = Old_Zebro_4_Middle_x - 200
                Old_Zebro_4_Middle_y_pos = Old_Zebro_4_Middle_y + 200
                Old_Zebro_4_Middle_y_neg = Old_Zebro_4_Middle_y - 200

                Old_Zebro_5_Middle_x_pos = Old_Zebro_5_Middle_x + 200
                Old_Zebro_5_Middle_x_neg = Old_Zebro_5_Middle_x - 200
                Old_Zebro_5_Middle_y_pos = Old_Zebro_5_Middle_y + 200
                Old_Zebro_5_Middle_y_neg = Old_Zebro_5_Middle_y - 200

                Old_Zebro_6_Middle_x_pos = Old_Zebro_6_Middle_x + 200
                Old_Zebro_6_Middle_x_neg = Old_Zebro_6_Middle_x - 200
                Old_Zebro_6_Middle_y_pos = Old_Zebro_6_Middle_y + 200
                Old_Zebro_6_Middle_y_neg = Old_Zebro_6_Middle_y - 200
                ...
                elif (x < Old_Zebro_3_Middle_x_pos) and (x > Old_Zebro_3_Middle_x_neg) and (y < Old_Zebro_3_Middle_y_pos) and (y > Old_Zebro_3_Middle_y_neg):
                    Zebro_3_Led = [x,y]
                    Zebro_3_Leds.append(Zebro_3_Led)
                elif (x < Old_Zebro_4_Middle_x_pos) and (x > Old_Zebro_4_Middle_x_neg) and (y < Old_Zebro_4_Middle_y_pos) and (y > Old_Zebro_4_Middle_y_neg):
                    Zebro_4_Led = [x,y]
                    Zebro_4_Leds.append(Zebro_4_Led)
                elif (x < Old_Zebro_5_Middle_x_pos) and (x > Old_Zebro_5_Middle_x_neg) and (y < Old_Zebro_5_Middle_y_pos) and (y > Old_Zebro_5_Middle_y_neg):
                    Zebro_5_Led = [x,y]
                    Zebro_5_Leds.append(Zebro_5_Led)
                elif (x < Old_Zebro_6_Middle_x_pos) and (x > Old_Zebro_6_Middle_x_neg) and (y < Old_Zebro_6_Middle_y_pos) and (y > Old_Zebro_6_Middle_y_neg):
                    Zebro_6_Led = [x,y]
                    Zebro_6_Leds.append(Zebro_6_Led)
                Led = [x,y]
                Leds.append(Led)
            
            for Zebros_1 in range(6):
                ...
                if Zebros_1 == 0:
                    ...
                    for Testing_i in Zebro_4_Leds:
                        for Testing_y in Zebro_4_Leds:
                            Delta_X = abs(Testing_i[0] - Testing_y[0])
                            Delta_Y = abs(Testing_i[1] - Testing_y[1])
                            Distance = math.sqrt((Delta_X*Delta_X) + (Delta_Y*Delta_Y))
                            if (Distance > 25) and (Distance <= 32):
                    ...
                    if (Zebro_4_Middle_x == 0) or (Zebro_2_Middle_y == 0):
                        Zebro_4_Middle_x = Old_Zebro_4_Middle_x
                        Zebro_4_Middle_y = Old_Zebro_4_Middle_y
                elif Zebros_1 == 4:
                    for Testing_i in Zebro_5_Leds:
                        for Testing_y in Zebro_5_Leds:
                            Delta_X = abs(Testing_i[0] - Testing_y[0])
                            Delta_Y = abs(Testing_i[1] - Testing_y[1])
                            Distance = math.sqrt((Delta_X*Delta_X) + (Delta_Y*Delta_Y))
                            if (Distance > 25) and (Distance <= 32):
                    ...
                    if (Zebro_5_Middle_x == 0) or (Zebro_5_Middle_y == 0):
                        Zebro_5_Middle_x = Old_Zebro_5_Middle_x
                        Zebro_5_Middle_y = Old_Zebro_5_Middle_y
                elif Zebros_1 == 5:
                    for Testing_i in Zebro_6_Leds:
                        for Testing_y in Zebro_6_Leds:
                            Delta_X = abs(Testing_i[0] - Testing_y[0])
                            Delta_Y = abs(Testing_i[1] - Testing_y[1])
                            Distance = math.sqrt((Delta_X*Delta_X) + (Delta_Y*Delta_Y))
                            if (Distance > 25) and (Distance <= 32):
                    ...
                    if (Zebro_6_Middle_x == 0) or (Zebro_6_Middle_y == 0):
                        Zebro_6_Middle_x = Old_Zebro_6_Middle_x
                        Zebro_6_Middle_y = Old_Zebro_6_Middle_y
            for Zebros in range(6):
                if Zebros == 0:
                    Blocking_Zebro = Block.Block_9(Zebro_1_Middle_x, Zebro_2_Middle_x, Zebro_3_Middle_x, Zebro_4_Middle_x, Zebro_5_Middle_x, Zebro_6_Middle_x,
                                                   Zebro_1_Middle_y, Zebro_2_Middle_y, Zebro_3_Middle_y, Zebro_4_Middle_y, Zebro_5_Middle_y, Zebro_6_Middle_y) 
...
if __name__ == '__main__':
    ...
    main(q_Control_Serial_Write,q_Data_is_Send,q_Control_Uart_Main,q_PicoZebro_1, q_PicoZebro_2,q_PicoZebro_3,q_PicoZebro_4,q_PicoZebro_5,q_PicoZebro_6,
         q_Pico_Direction_1, q_Pico_Direction_2, q_Pico_Direction_3,q_Pico_Direction_4,q_Pico_Direction_5,q_Pico_Direction_6,
         q_Pico_Angle_1, q_Pico_Angle_2, q_Pico_Angle_3, q_Pico_Angle_4, q_Pico_Angle_5, q_Pico_Angle_6)
