...
int32_t address_measure_position(void){
	static uint32_t adc_readout;
	static uint32_t adc_readout_temp;
	static uint32_t adc_readout_mean;
	int32_t cursor;
...
//	    		2458, INT32_MAX, 3143, INT32_MAX, 3724, INT32_MAX};
	...
	for (i = 0; i<64; i++) {
		adc_readout_temp = (adc_get_value(ADC_ID_RESISTOR_INDEX));
		adc_readout += adc_readout_temp;
		while (adc_get_value(ADC_ID_RESISTOR_INDEX) == adc_readout_temp) {
			/* wait for new data */
		}
	}

	/* left aligned 10 bit, so shift! */
	adc_readout_mean = (adc_readout / 64)>>4;
	/* find the closest element in the adc_values array */
	for(cursor = 0; cursor < ADDRESS_NUMBER_OF_POSITIONS; cursor++){
		int32_t distance;
		distance = abs(adc_values[cursor] - adc_readout_mean);
