...
void fans_init(void) {
	...
	TIM15->CCMR1 |= TIM_CCMR1_OC1M_2 | TIM_CCMR1_OC1M_1 | TIM_CCMR1_OC2M_2
			| TIM_CCMR1_OC2M_1;
...
int32_t fans_calc_speed(int32_t fan) {
	int32_t speed;
	int32_t sensor;
	int32_t highest_measured_temperature = 0; // Celcius
	// There is one internal thermometer
	int32_t temperature[(POOTBUS_NUM_OF_TEMP_SENSORS + 1)];

	speed = 0;
	// Store the internal thermometer temperature in the last place of temperature
	temperature[POOTBUS_NUM_OF_TEMP_SENSORS] = adc_get_temperature();

	for (sensor = 0; sensor <= POOTBUS_NUM_OF_TEMP_SENSORS; sensor++) {
		if (sensor < POOTBUS_NUM_OF_TEMP_SENSORS) {
			temperature[sensor] = pootbus_get_motor_temperature_fp3(sensor)
					/ 1000;
		}
		if (sensor == 0) {
			highest_measured_temperature = temperature[sensor];
		}
		if (sensor != 0 && (temperature[sensor - 1] <= temperature[sensor])) {
			highest_measured_temperature = temperature[sensor];
		}
	}
	...
	if (highest_measured_temperature >= FANS_ON_TEMPERATURE) {
		speed = FANS_MAX_SPEED
				* (highest_measured_temperature
						/ (0.5 * POOTBUS_EMERGENCY_MOTOR_TEMPERATURE));
