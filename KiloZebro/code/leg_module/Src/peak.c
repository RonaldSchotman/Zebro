...
uint8_t peak_process_adc_values_sensor(void) {
	if (time_get_time_ms() % 50 == 0) { //if this loop is too fast, peak detected does not stay high long enough and doesn't work because delta becomes too small. If too slow, the detection rate is too slow.
	uint16_t adc_data;
	...
	static uint16_t max_avg_delta = 0;
